...
`;
  ...
  try {
    ...
    await sendTraceToOpik("judge_evaluation_failure", {
      ...
      output: {
        status: verdict.status,
        integrity_score: verdict.integrityScore,
        effort_score: verdict.effortScore,
        safety_score: verdict.safetyScore,
        overall_score: (verdict.integrityScore + verdict.effortScore + verdict.safetyScore) / 3,
        final_xp: verdict.finalXp,
