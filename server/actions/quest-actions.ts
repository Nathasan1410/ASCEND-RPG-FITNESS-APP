  ...
  try {
    ...
    await sendTraceToOpik("quest_generation_fallback", {
      ...
      output: {
        ...
        exercise_names: fallbackPlan.exercises.map((e: any) => e.name),
        ...
        quest_narrative: fallbackPlan.narrative_intro?.substring(0, 200) + "...",
        stat_gain: fallbackPlan.stat_gain,
      ...
      tags: [
        ...
        profile.rank_tier,
        profile.class,
        fallbackPlan.quest_rank,
        fallbackPlan.quest_type,
        err?.name ? `error_${err.name}` : undefined,
      ].filter(Boolean),
    });
